...
def draw_window(win, birds, pipes, base, score):
    ...
    text = STAT_FONT.render("Score: "+str(score)+" | Birds Alive: "+str(len(birds)),1,(255,255,255))
...
def main(genomes, config):
    ...
    fitnesses = [1e-10 for _ in range(len(dtraj))]
    for i in range(len(dtraj)):
    ...
    if max(fitnesses)>(mean(fitnesses)+3.5*(std_dev(fitnesses))) and end-start>30:
        shouldExit = True
    else:
        print("SHOULD NOT STOP")

    print("Current threshold: ", (max(fitnesses)-mean(fitnesses))/std_dev(fitnesses))
